...
function EditQuestion(props) {
    ...
    const { token, subjectsData, getSubjectsData, putQuestion } = props

    const { _id, diffLevel, options, questionText, rightMarks, subject, topic, type, wrongMarks } = location.state
    ...
    useEffect(() => {
        getSubjectsData()
        axios.get(`http://admin.liveexamcenter.in/api/topics/subject/${data.subject}`,
            {
                headers: {
                    'Authorization': token
                }
        ...
        setQuestionTextRTE(isHTML(questionText))
        setData(prevData => {
            return {
                ...prevData,
                options: prevData.options.map(option => {
                    return {
                        ...option,
                        richTextEditor: isHTML(option.option)
                    }
                })
            }
        })
    ...
    function optionsValueHandler(e, index) {
        console.log("option " + index + ' called')
        validateData(e)
    ...
    function submitQuestion(e) {
        ...
        if (isUniqueOptions && answers.length > 0) {
            const result = putQuestion(_id, data)
            if (result) {
                // alert("Success")
    ...
    function cancelEdit() {
        toast.info('Editing question cancelled!', {
            position: "bottom-right",
            autoClose: 5000,
            hideProgressBar: false,
            closeOnClick: true,
    ...
    const subjectElements = subjectsData && subjectsData.map(subject => <option key={subject._id} value={subject._id} selected={subject._id === data.subject}>{subject.name}</option>)

    const topicElements = topicsData && topicsData.map(topic => <option key={topic._id} value={topic._id} selected={topic._id === data.topic}>{topic.name}</option>)
    ...
    return (
        <div className='border rounded-3' style={{ margin: '60px 120px' }}>
            <form /* className={`${allDataFilled ? 'was-validated' : ''}`} */ onSubmit={(e) => submitQuestion(e)} autoCorrect='off' autoComplete='off'>
                ...
                </div>                {/* <div className='container' style={{ padding: '20px' }}> */}
                <div className="row g-3" style={{ padding: '20px', color: 'rgb(106 112 112)' }}>
                    <div className="col-md-6">
                        ...
                        <div id="validationSubjectFeedback" class="invalid-feedback">
                            Subject is required.
                        </div>
                    </div>
                    <div className="col-md-6">
                        ...
                        <div id="validationTopicFeedback" class="invalid-feedback">
                            Topic is required.
                        </div>
                    </div>
                    <div className="col-md-3">
                        ...
                        <select className={`form-select ${allDataFilled && data.type === '' && 'is-invalid'}`} aria-label="Default select example" name='type' aria-describedby="validationTypeFeedback" onChange={(e) => dataHandler(e)} onBlur={(e) => validateData(e)} defaultValue={data.type} required>
                    ...
                    <div className="col-md-3">
                        ...
                        {/* <input type="text" className="form-control" id="inputAddress" placeholder="1234 Main St" /> */}
                        <select className={`form-select ${allDataFilled && data.diffLevel === '' && 'is-invalid'}`} aria-label="Default select example" name='diffLevel' aria-describedby="validationDiffLevelFeedback" onChange={(e) => dataHandler(e)} onBlur={(e) => validateData(e)} defaultValue={data.diffLevel} required>
                            <option value='' selected>Select level ...</option>
                            <option value="Easy">Easy</option>
                            <option value="Medium">Medium</option>
                            <option value="Hard">Hard</option>
                        </select>
                        <div id="validationDiffLevelFeedback" class="invalid-feedback">
                    ...
                    <div className="col-md-3">
                        ...
                        <div id="validationWrongMarksFeedback" class="invalid-feedback">
                    ...
                    {
                        isUniqueOptions ?
                            allDataFilled && data.options.filter(option => option.option !== '').length === data.options.length && answers.length === 0 && <div style={{ color: 'red' }}>Please select correct answer from options.</div> :
                ...
                <div className='border-top' style={{ margin: '0px', padding: '20px', textAlign: 'left', backgroundColor: '#f1f1f1' }}>
                    <button type="submit" className="btn btn-primary" onClick={() => submitButton()}>Update question</button>
                    <button type="button" className="btn btn-outline-secondary" style={{ marginLeft: '20px' }} onClick={() => cancelEdit()}>Cancel</button>
                </div>
            </form>
        </div>
    )
